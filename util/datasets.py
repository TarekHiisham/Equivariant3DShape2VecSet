...
def build_shape_surface_occupancy_dataset(split, args):
    if split == 'train':
        ...
        return ShapeNet(args.mesh_folder, args.point_folder,
                        split=split, transform=transform, sampling=True, num_samples=1024, return_surface=True, surface_sampling=True, pc_size=args.point_cloud_size)
    elif split == 'val':
        ...
        return ShapeNet(args.mesh_folder, args.point_folder,
                        split=split, transform=None, sampling=False, return_surface=True, surface_sampling=True, pc_size=args.point_cloud_size)
    else:
        return ShapeNet(args.mesh_folder, args.point_folder,
                        split=split, transform=None, sampling=False, return_surface=True, surface_sampling=True, pc_size=args.point_cloud_size)
